...
class ParallelConfig:
    ...
    distributed_executor_backend: Optional[Union[str, Type["ExecutorBase"]]] = (
        None
    )
...
class KVTransferConfig(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        if all(
            ...
            raise ValueError(
                f"Unsupported kv_connector: {self.kv_connector}. "
...
class CompilationConfig(BaseModel):
    ...
    def init_with_cudagraph_sizes(self, sizes_to_specialize: List[int]):
        ...
        ignored_sizes = [
            x
            for x in self.candidate_compile_sizes
...
class VllmConfig:
    ...
    parallel_config: ParallelConfig = field(
        default_factory=ParallelConfig, init=True
    )
    ...
    compilation_config: CompilationConfig = field(default=None, init=True)  # type: ignore
    kv_transfer_config: KVTransferConfig = field(default=None, init=True)  # type: ignore
    instance_id: str = ""
    ...
    def __post_init__(self):
        ...
        if not envs.VLLM_USE_V1:
            ...
            batch_size_capture_list = [
                size
                for size in _BATCH_SIZES_TO_CAPTURE
